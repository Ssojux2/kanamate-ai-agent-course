...
이 문서는 카나메이트 6주 과정을 학습 노트북, 주차별 Python 실습 파일, Gradio WebUI로 분리한 현재 구조를 설명한다.
...
exercises/
  week01_practice.py ... week06_practice.py
kanamate_runtime/
  common.py
  week01.py ... week06.py
gradio_apps/
  week01_app.py ... week06_app.py
tests/
  test_week01_practice.py ... test_week06_practice.py
```
...
- 노트북은 학습 흐름과 실행 예시를 제공하고, 코드 작성형 실습은 `exercises/weekXX_practice.py`에서 실행한다.
- Gradio 앱은 `python -m gradio_apps.weekXX_app` 명령으로 실행한다.
- private 강의 repo 기준으로 모범 답안 주석과 구현은 실습 파일에 유지한다.
...
| 6. 코드 작성형 실습(.py 파일) | 주차별 `exercises/weekXX_practice.py` helper를 실행 |
| 6-0. 실습 자동 점검 | 모델 문구가 아니라 trace, structured response, payload를 assert로 확인 |
| 6-1. 로컬 Gradio UI 실습 | `gradio_apps/weekXX_app.py`로 같은 결과를 WebUI에서 실행 |
...
- `python -m compileall exercises kanamate_runtime gradio_apps`로 Python 문법을 확인한다.
- `pytest`로 주차별 practice 함수의 구조화 결과, trace 파싱, payload 키를 검증한다.
- 노트북 JSON 유효성을 검사한다.
...
- 각 주차 실습은 `exercises/weekXX_practice.py`에서 실행된다.
- 각 주차 WebUI는 `gradio_apps/weekXX_app.py`에서 실행된다.
- `.env`는 Git에 포함하지 않고 `.env.example`만 제공한다.
- private GitHub repo `Ssojux2/kanamate-ai-agent-course`에 올릴 수 있는 README, requirements, tests, ignore 파일이 준비되어 있다.
