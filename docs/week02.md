...
1. `notebook/02_structured_output_pydantic.ipynb`에서 `response_format`이 Pydantic 모델을 검증하는 흐름을 본다.
2. `week02.py`의 `ScheduleCreate`, `TodoCreate`, `ReminderCreate`, `PracticeExtractionResult`를 확인한다.
3. 일정, 할 일, 알림, 애매한 질문을 각각 넣어 `kind`가 어떻게 달라지는지 본다.
4. `structured_payload_to_message`가 구조화 결과를 사용자 답변으로 바꾸는 방식을 확인한다.
5. `python week02.py`로 Gradio UI를 실행해 payload와 채팅 답변을 비교한다.
